...
public class LCO_MInvoice extends MInvoice
{
	...
	public int recalcWithholdings() {
		...
		//amount of document 
		BigDecimal amtbase = getTotalLines();
		
		// Search withholding types applicable depending on IsSOTrx
		List<X_LCO_WithholdingType> wts = new Query(getCtx(), X_LCO_WithholdingType.Table_Name, " IsRecalcInPayment='N' and IsSOTrx=?", get_TrxName())
		...
		for (X_LCO_WithholdingType wt : wts)
		{
			...
			/*
			 * Update from withholdig venezuela 
			 * by vcappugi
			*/
			//taxidtype
			if (wrc.isUseTxIDType()) {
				int taxidtype = DB.getSQLValue(get_TrxName(),"SELECT (SELECT LCO_TaxIdType_ID from C_BPartner bp1 where bp1.C_BPartner_id=  inv.C_BPartner_ID) "
					...
					if (taxidtype>0)
						wherer.append(" AND LCO_TaxIDType_ID =" + taxidtype);
			}
			//municipality
			if (wrc.isUseMunicipality()) {
				  // Include Municipality in withholding calc
					...
					if (municipality > 0)
						wherer.append(" AND C_Municipality_ID =" + municipality);
				}
			//product ciuu
			if (wrc.IsUseProduct_CIU()) {
				// look the conf fields
				String sqlwctp =
					"SELECT DISTINCT COALESCE (p.LCO_ISIC_ID, COALESCE (c.LCO_ISIC_ID, 0)) "
					+ "  FROM C_InvoiceLine il "
					+ "  LEFT OUTER JOIN M_Product p ON il.M_Product_ID = p.M_Product_ID "
					+ "  LEFT OUTER JOIN C_Charge c ON (il.C_Charge_ID = c.C_Charge_ID) "
					+ "  WHERE C_Invoice_ID = ? AND il.IsActive='Y'";
				PreparedStatement pstmtwct = DB.prepareStatement(sqlwctp, get_TrxName());
				ResultSet rswct=null;
				try {
					pstmtwct.setInt(1, getC_Invoice_ID());
					rswct= pstmtwct.executeQuery();
						...
						while (rswct.next()) {
							...
							if (wcid > 0) {
								if (i == 0) {
									wherer.append(" AND LCO_Product_ISIC_ID IN (");
									addedlines = true;
								} else {
									wherer.append(",");
								}
			...
			if (wrc.isUseAmount()) {
				...
				wherer.append(" AND LCO_AmountFrom<=? AND LCO_AmountTo >= ?");
				paramsr.add(amtbase);
				paramsr.add(amtbase);
			}
				...
				if (wc.getBaseType() == null) {
					...
					if (wrc.isUseWithholdingCategory() && wrc.isUseProductTaxCategory()) {
						...
						sqllca = 
								...
								+ " WHERE il.IsActive='Y' AND  il.C_Invoice_ID= ?  AND (p.LCO_ISIC_ID = ? or c.LCO_ISIC_ID=? )";
						paramslca.add(wr.getLCO_Product_ISIC_ID());
						paramslca.add(wr.getLCO_Product_ISIC_ID());
					}
					else {
						// base = all lines
